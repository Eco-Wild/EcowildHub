    ...
    <>
      ...
      <section>
        <div className='fixed top-0 bottom-0 right-0 left-0 w-[240px] h-full font-Montserrat bg-tertiary-600 p-5 pt-1 text-white md:block hidden'>
          <div>
            <div
              className='inline-flex items-center gap-2 border-b border-[#2b3248] py-1 px-2 -ml-2'
              role='sidebar'
            ...
            >
              <img
                ...
                className='w-12 h-12'
              />
              <h2 className='font-semibold gap-4 text-sm' id=''>
                Ecowild Hub
              </h2>
            </div>
            <ul className='flex flex-col  mt-6'>
              {menuList.map((menu, index) => {
                const isActive = menu.link === location.pathname;
                return (
                  <li
                    key={index}
                    className={clsx(
                      'flex-1 py-4 px-2 font-medium text-sm rounded-lg ',
                      isActive
                        ? 'bg-[#142e84]'
                        : 'hover:bg-[#142e84] hover:bg-opacity-50'
                    )}
                  >
